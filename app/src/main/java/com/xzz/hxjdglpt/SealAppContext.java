...
import com.xzz.hxjdglpt.activity.ui.activity.NewFriendListActivity;
...
import com.xzz.hxjdglpt.message.module.SealExtensionModule;
...
import com.xzz.hxjdglpt.server.response.ContactNotificationMessageData;
 ...
 * Company RongCloud
 */
public class SealAppContext implements RongIM.ConversationListBehaviorListener, RongIMClient
        .OnReceiveMessageListener, RongIM.UserInfoProvider, RongIM.GroupInfoProvider, RongIM
        .GroupUserInfoProvider, RongIM.LocationProvider, RongIMClient.ConnectionStatusListener,
        RongIM.ConversationBehaviorListener, RongIM.IGroupMembersProvider {
    ...
    private void initListener() {
        ...
        RongIM.getInstance().setSendMessageListener(new RongIM.OnSendMessageListener() {
            ...
            @Override
            public boolean onSent(Message message, RongIM.SentMessageErrorCode
                    sentMessageErrorCode) {
                if (message.getSentStatus() == Message.SentStatus.SENT) {
                    LogUtil.i("message getExtra=" + message.getExtra());
                    LogUtil.i("message getObjectName=" + message.getObjectName());
                    LogUtil.i("message getSenderUserId=" + message.getSenderUserId());
                    LogUtil.i("message getTargetId=" + message.getTargetId());
                    LogUtil.i("message getContent=" + message.getContent());
                    LogUtil.i("message getUId=" + message.getUId());
                    LogUtil.i("message getMessageDirection=" + message.getMessageDirection());
                    LogUtil.i("message getMessageId=" + message.getMessageId());
                    LogUtil.i("message getConversationType=" + message.getConversationType());
                    LogUtil.i("SENT");
                    MessageContent messageContent = message.getContent();

                    if (messageContent instanceof TextMessage) {//文本消息
                        TextMessage textMessage = (TextMessage) messageContent;
                        LogUtil.i("onSent-TextMessage:" + textMessage.getContent());
                        sendMessage(message.getSenderUserId(), message.getTargetId(), textMessage
                                .getContent());
                    } else if (messageContent instanceof ImageMessage) {//图片消息
                        ImageMessage imageMessage = (ImageMessage) messageContent;
                        LogUtil.i("onSent-ImageMessage:" + imageMessage.getRemoteUri());
                    } else if (messageContent instanceof VoiceMessage) {//语音消息
                        VoiceMessage voiceMessage = (VoiceMessage) messageContent;
                        LogUtil.i("onSent-voiceMessage:" + voiceMessage.getUri().toString());
                    } else if (messageContent instanceof RichContentMessage) {//图文消息
                        RichContentMessage richContentMessage = (RichContentMessage) messageContent;
                        LogUtil.i("onSent-RichContentMessage:" + richContentMessage.getContent());
                    } else {
                        LogUtil.i("onSent-其他消息，自己来判断处理");
                    }
                } else if (message.getSentStatus() == Message.SentStatus.FAILED) {
                    LogUtil.i("FAILED");
                } else if (message.getSentStatus() == Message.SentStatus.CANCELED) {
                    LogUtil.i("CANCELED");
    ...
    private void sendMessage(String senderId, String targetId, String content) {
        String url = ConstantUtil.BASE_URL + "/m_mi/sendMessage";
        HashMap<String, String> params = new HashMap<>();
        params.put("senderId", senderId);
        params.put("targetId", targetId);
        params.put("content", content);
        VolleyRequest.RequestPost(mContext, url, "sendMessage", params, new
    ...
    private void setReadReceiptConversationType() {
        Conversation.ConversationType[] types = new Conversation.ConversationType[]{Conversation
    ...
    private void setInputProvider() {
        ...
        List<IExtensionModule> moduleList = RongExtensionManager.getInstance()
                .getExtensionModules();
    ...
    public boolean onConversationPortraitClick(Context context, Conversation.ConversationType
            conversationType, String s) {
        return false;
    }

    @Override
    public boolean onConversationPortraitLongClick(Context context, Conversation.ConversationType
            conversationType, String s) {
        return false;
    }
    ...
    public boolean onConversationClick(Context context, View view, UIConversation uiConversation) {
        ...
        if (messageContent instanceof ContactNotificationMessage) {
            ContactNotificationMessage contactNotificationMessage = (ContactNotificationMessage)
                    messageContent;
            if (contactNotificationMessage.getOperation().equals("AcceptResponse")) {
                ...
                if (contactNotificationMessage.getExtra() != null) {
                    ...
                    try {
                        bean = JsonMananger.jsonToBean(contactNotificationMessage.getExtra(),
                                ContactNotificationMessageData.class);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                    RongIM.getInstance().startPrivateChat(context, uiConversation
                            .getConversationSenderId(), bean.getSourceUserNickname());
    ...
    public boolean onReceived(Message message, int i) {
        ...
        if (messageContent instanceof ContactNotificationMessage) {
            ...
            if (contactNotificationMessage.getOperation().equals("Request")) {
                ...
                try {
                    contactNotificationMessageData = JsonMananger.jsonToBean
                ...
                if (contactNotificationMessageData != null) {
                    if (SealUserInfoManager.getInstance().isFriendsRelationship
                            (contactNotificationMessage.getSourceUserId())) {
                        return false;
                    }
                    SealUserInfoManager.getInstance().addFriend(new Friend
                            (contactNotificationMessage.getSourceUserId(),
                                    contactNotificationMessageData.getSourceUserNickname(), null,
                                    CharacterParser.getInstance().getSpelling
...
//                    SealUserInfoManager.getInstance().getGroupMember(groupID);
                    ...
                    if (data != null) {
                            ...
                            for (String userId : memberIdList) {
                                if (currentID.equals(userId)) {
                                    RongIM.getInstance().removeConversation(Conversation
                                            .ConversationType.GROUP, message.getTargetId(), new
                                            RongIMClient.ResultCallback<Boolean>() {
                                        @Override
                                        public void onSuccess(Boolean aBoolean) {
                                            Log.e("SealAppContext", "Conversation remove " +
                                                    "successfully.");
                                        }
                        ...
                        SealUserInfoManager.getInstance().deleteGroupMembers(groupID,
                                kickedUserIDs);
                        BroadcastManager.getInstance(mContext).sendBroadcast(UPDATE_GROUP_MEMBER,
                                groupID);
                    }
                } else if (groupNotificationMessage.getOperation().equals("Add")) {
//                    SealUserInfoManager.getInstance().addGroup(groupID);
                    addGroups(groupID);
//                    SealUserInfoManager.getInstance().getGroupMember(groupID);
                    BroadcastManager.getInstance(mContext).sendBroadcast(UPDATE_GROUP_MEMBER,
                            groupID);
                    ...
                    if (data != null) {
                        ...
                        SealUserInfoManager.getInstance().updateGroupsName(groupID,
                                targetGroupName);
                        List<String> groupNameList = new ArrayList<>();
                        groupNameList.add(groupID);
                        groupNameList.add(data.getTargetGroupName());
                        groupNameList.add(data.getOperatorNickname());
                        BroadcastManager.getInstance(mContext).sendBroadcast(UPDATE_GROUP_NAME,
                                groupNameList);
    ...
    private void handleGroupDismiss(final String groupID) {
        RongIM.getInstance().getConversation(Conversation.ConversationType.GROUP, groupID, new
                RongIMClient.ResultCallback<Conversation>() {
            @Override
            public void onSuccess(Conversation conversation) {
                RongIM.getInstance().clearMessages(Conversation.ConversationType.GROUP, groupID,
                        new RongIMClient.ResultCallback<Boolean>() {
                    @Override
                    public void onSuccess(Boolean aBoolean) {
                        RongIM.getInstance().removeConversation(Conversation.ConversationType
                                .GROUP, groupID, null);
                    }
...
//            context.startActivity(intent);
    ...
    public void setLastLocationCallback(RongIM.LocationProvider.LocationCallback
    ...
    public void popAllActivity() {
        try {
            if (MainMIActivity.mViewPager != null) {
                MainMIActivity.mViewPager.setCurrentItem(0);
            }
...
//        SharedPreferences.Editor editor = mContext.getSharedPreferences("config", Context
//                .MODE_PRIVATE).edit();
//        if (!isKicked) {
...
//        loginActivityIntent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK | Intent
...
//        mContext.startActivity(loginActivityIntent);
    ...
    public void getGroupMembers(String groupId, final RongIM.IGroupMemberCallback callback) {
        SealUserInfoManager.getInstance().getGroupMembers(groupId, new SealUserInfoManager
                .ResultCallback<List<GroupMember>>() {
            @Override
            public void onSuccess(List<GroupMember> groupMembers) {
                ...
                if (groupMembers != null) {
                    for (GroupMember groupMember : groupMembers) {
                        if (groupMember != null) {
                            UserInfo userInfo = new UserInfo(groupMember.getUserId(), groupMember
                                    .getName(), groupMember.getPortraitUri());
    ...
    private void addGroups(String groupId) {

        String url = ConstantUtil.BASE_URL + "/m_group/queryGroupById";
        HashMap<String, String> params = new HashMap<>();
        params.put("id", groupId);
        VolleyRequest.RequestPost(mContext, url, "queryGroupById", params, new
                VolleyListenerInterface(mContext, VolleyListenerInterface.mListener,
                        VolleyListenerInterface.mErrorListener) {
...
//                SuccinctProgress.dismiss();
                try {
                    ...
                    if ("1".equals(resultCode)) {
                        ...
                        Gson gson = new Gson();
                        Groups groups = (Groups) gson.fromJson(result.getJSONObject("data")
                                .toString(), Groups.class);
                        SealUserInfoManager.getInstance().addGroup(groups);
                        Group groupInfo;
                        if (groups != null) {
                            groupInfo = new Group(groups.getId(), groups.getName(), Uri.parse
                                    (ConstantUtil.FILE_DOWNLOAD_URL + groups.getPortraitUri()));
