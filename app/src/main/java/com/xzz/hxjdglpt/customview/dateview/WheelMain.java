package com.xzz.hxjdglpt.customview.dateview;
...
//		}
            ...
            public void onChanged(WheelView wheel, int oldValue, int newValue) {
                ...
                if (list_big.contains(String.valueOf(month_num))) {
                    ...
                    if (((wv_year.getCurrentItem() + START_YEAR) % 4 == 0 && (wv_year
                            .getCurrentItem() + START_YEAR) % 100 != 0) || (wv_year
                            .getCurrentItem() + START_YEAR) % 400 == 0)
                        wv_day.setAdapter(new NumericWheelAdapter(1, 29));
                    else wv_day.setAdapter(new NumericWheelAdapter(1, 28));
                }
            }
        ...
        int textSize = 0;
        if (hasSelectTime) textSize = (screenheight / 140) * 4;
        else textSize = (screenheight / 140) * 4;
    ...
    public String getTime() {
        ...
        int minute = 0;
        if (wv_mins != null) {
            minute = Integer.valueOf(adapter.getItem(wv_mins.getCurrentItem()));
        }
        if (month <= 9) {
            strMon = "0" + month;
        } else {
        ...
        if (!hasSelectTime) {
            ...
            sb.append((wv_year.getCurrentItem() + START_YEAR)).append("-").append(strMon).append
...
//						.append((wv_month.getCurrentItem() + 1)).append(wv_hours.getCurrentItem())
// .append(":").append(wv_mins.getAdapter().getItem(wv_mins.getCurrentItem()));
//			} else {
...
//						.append((wv_day.getCurrentItem() + 1)).append(wv_hours.getCurrentItem())
// .append(":").append(wv_mins.getAdapter().getItem(wv_mins.getCurrentItem()));
//			} else {
//				sb.append((wv_year.getCurrentItem() + START_YEAR)).append("-")
//						.append((wv_month.getCurrentItem() + 1)).append("-")
