...
import com.xzz.hxjdglpt.db.FriendDao;
import com.xzz.hxjdglpt.db.GroupMember;
import com.xzz.hxjdglpt.db.GroupMemberDao;
import com.xzz.hxjdglpt.db.Groups;
import com.xzz.hxjdglpt.db.GroupsDao;
import com.xzz.hxjdglpt.db.UserInfoBean;
import com.xzz.hxjdglpt.server.SealAction;
import com.xzz.hxjdglpt.server.network.async.AsyncTaskManager;
import com.xzz.hxjdglpt.server.network.async.OnDataListener;
import com.xzz.hxjdglpt.server.network.http.HttpException;
import com.xzz.hxjdglpt.server.pinyin.CharacterParser;
import com.xzz.hxjdglpt.server.response.GetBlackListResponse;
import com.xzz.hxjdglpt.server.response.GetGroupInfoResponse;
import com.xzz.hxjdglpt.server.response.GetGroupMemberResponse;
import com.xzz.hxjdglpt.server.response.GetGroupResponse;
import com.xzz.hxjdglpt.server.response.GetTokenResponse;
import com.xzz.hxjdglpt.server.response.UserRelationshipResponse;
    ...
    public void getUserInfo(final String userId) {
        ...
        mWorkHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                friend.setPortraitUri(Uri.parse(ConstantUtil.FILE_DOWNLOAD_URL + friend
                        .getDiaplayPic()));
                friend.setName(friend.getDisplayName());
                if (friend != null) {
    ...
    public void getGroupInfo(final String groupsId) {
        ...
        mWorkHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                if (group != null) {
                    groupInfo = new Group(groupsId, group.getName(), Uri.parse(ConstantUtil
                            .FILE_DOWNLOAD_URL + group.getPortraitUri()));
    ...
    public void setUserInfoEngineListener() {
        UserInfoEngine.getInstance(mContext).setListener(new UserInfoEngine.UserInfoListener() {
            ...
            public void onResult(UserInfo info) {
                if (info != null && RongIM.getInstance() != null) {
                    if (TextUtils.isEmpty(info.getPortraitUri() == null ? null : info
                            .getPortraitUri().toString())) {
                        info.setPortraitUri(Uri.parse(RongGenerate.generateDefaultAvatar(info
                                .getName(), info.getUserId())));
                    }
                    NLog.d(TAG, "SealUserInfoManager getUserInfo from network " + info.getUserId
                            () + " " + info.getName() + " " + info.getPortraitUri());
        ...
        GroupInfoEngine.getInstance(mContext).setmListener(new GroupInfoEngine.GroupInfoListeners
            ...
            public void onResult(Group info) {
                if (info != null && RongIM.getInstance() != null) {
                    NLog.d(TAG, "SealUserInfoManager getGroupInfo from network " + info.getId() +
                            " " + info.getName() + " " + info.getPortraitUri());
                    if (TextUtils.isEmpty(info.getPortraitUri() == null ? null : info
                            .getPortraitUri().toString())) {
                        info.setPortraitUri(Uri.parse(RongGenerate.generateDefaultAvatar(info
...
//                    e.printStackTrace();
//                    RLog.d(TAG, "fetchUserInfo occurs HttpException e=" + e.toString() +
//                            "mGetAllUserInfoState=" + mGetAllUserInfoState);
//                } catch (Exception e) {
//                    e.printStackTrace();
//                    RLog.d(TAG, "fetchUserInfo occurs Exception e=" + e.toString() +
//                            "mGetAllUserInfoState=" + mGetAllUserInfoState);
//                }
//                setGetAllUserInfoDone();
//            }
...
//            List<UserRelationshipResponse.ResultEntity> list = userRelationshipResponse
// .getResult();
...
//                            GetGroupInfoResponse.ResultEntity groupInfo = groupInfoResponse
//                                    .getResult();
//                            if (groupInfo != null) {
//                                String role = groupInfo.getCreatorId().equals(RongIM.getInstance
//                                        ().getCurrentUserId()) ? "0" : "1";
//                                syncAddGroup(new Groups(groupID, groupInfo.getName(), groupInfo
//                                        .getPortraitUri(), role));
//                            }
//                        } else {
//                            mGetAllUserInfoState &= ~GROUPS;
...
//                    e.printStackTrace();
//                    mGetAllUserInfoState &= ~GROUPS;
//                    NLog.d(TAG, "fetchGroups occurs HttpException e=" + e.toString() + "groupID="
//                            + groupID);
//                } catch (JSONException e) {
//                    e.printStackTrace();
//                    NLog.d(TAG, "fetchGroups occurs JSONException e=" + e.toString() + "groupID="
//                            + groupID);
//                }
//                sp.edit().putInt("getAllUserInfoState", mGetAllUserInfoState).apply();
//            }
...
//                    if (groupID.equals(groups.getGroupId())) group = groups;
//                }
//            }
//            mGetAllUserInfoState |= GROUPS;
//            return group;
...
//                } catch (JSONException e) {
//                    NLog.d(TAG, "fetchGroupMembers occurs JSONException e=" + e.toString() + ", "
...
//                    fetchGroupCount++;
//                    List<GetGroupMemberResponse.ResultEntity> list = groupMemberResponse
//                            .getResult();
//                    if (list != null && list.size() > 0) {
...
//                        setGetAllUserInfoWithPartGroupMembersState();
//                        sp.edit().putInt("getAllUserInfoState", mGetAllUserInfoState).apply();
//                        NLog.d(TAG, "getGroupMember occurs HttpException e=" + e.toString() +
//                                "groupID=" + groupID);
//                        return;
//                    } catch (JSONException e) {
//                        e.printStackTrace();
//                        NLog.d(TAG, "getGroupMember occurs JSONException e=" + e.toString() +
//                                "groupID=" + groupID);
//                        return;
//                    }
//                    if (groupMemberResponse != null && groupMemberResponse.getCode() == 200) {
...
//            syncDeleteGroupMembers();
//            for (Groups group : mGroupsList) {
        GetGroupMemberResponse groupMemberResponse = null;
        try {
            groupMemberResponse = action.getGroupMember(groupsID);
        } catch (JSONException e) {
            NLog.d(TAG, "pullGroupMembers(String groupsID) occurs JSONException e=" + e.toString
                    () + ", groupID=" + groupsID);
            fetchGroupCount++;
//                    continue;
        }
        if (groupMemberResponse != null && groupMemberResponse.getCode() == 200) {
            ...
            if (list != null && list.size() > 0) {
                List<GroupMember> memberList = addGroupMembers(list, groupsID);
                if (groupsID.equals(groupsID)) {
    ...
    public void addFriend(final Friend friend) {
        mWorkHandler.post(new Runnable() {
            ...
            public void run() {
                if (mFriendDao != null) {
                    if (friend != null) {
                        if (friend.getPortraitUri() == null || TextUtils.isEmpty(friend
                                .getPortraitUri().toString())) {
...
//                    portrait = RongGenerate.generateDefaultAvatar(groups.getName(), groups
//                            .getGroupId());
...
////                    Friend friend = new Friend(resultEntity.getUser().getId(), resultEntity
////                            .getUser().getNickname(), Uri.parse(resultEntity.getUser()
////                            .getPortraitUri()), resultEntity.getDisplayName(), null, null, null,
////                            null, CharacterParser.getInstance().getSpelling(resultEntity.getUser
////                            ().getNickname()), CharacterParser.getInstance().getSpelling
...
//    }
     ...
     * 添加朋友保存本地
     */
    public void addMyGroups(List<Groups> groupses) {
        if (groupses != null && groupses.size() > 0) {
            if (groupses.size() > 0) {
                if (mGroupsDao != null) {
                    mGroupsDao.insertOrReplaceInTx(groupses);
                }
            }
        }
    }
...
//                if (TextUtils.isEmpty(portrait)) {
//                    portrait = RongGenerate.generateDefaultAvatar(groups.getGroup().getName(),
//                            groups.getGroup().getId());
//                }
//                mGroupsList.add(new Groups(groups.getGroup().getId(), groups.getGroup().getName()
//                        , portrait));
//                ;
//            }
...
//    }
     ...
     */
    private List<GroupMember> addGroupMembers(final List<GetGroupMemberResponse.ResultEntity>
                                                      list, final String groupID) {
        if (list != null && list.size() > 0) {
            ...
            if (groupsMembersList != null && groupsMembersList.size() > 0) {
                for (GroupMember groupMember : groupsMembersList) {
                    if (groupMember != null && (groupMember.getPortraitUri() == null || TextUtils
                            .isEmpty(groupMember.getPortraitUri().toString()))) {
...
//                    } catch (HttpException e) {
//                        onCallBackFail(callback);
//                        NLog.d(TAG, "getFriends occurs HttpException e=" + e.toString() +
//                                "mGetAllUserInfoState=" + mGetAllUserInfoState);
//                        return;
//                    }
//                } else {
...
//                    } catch (HttpException e) {
//                        onCallBackFail(callback);
//                        NLog.d(TAG, "getGroups occurs HttpException e=" + e.toString() +
//                                "mGetAllUserInfoState=" + mGetAllUserInfoState);
//                        return;
//                    }
//                } else {
...
//    }
     ...
     */
    public void getGroupMembers(final String groupID, final ResultCallback<List<GroupMember>>
            callback) {
        if (TextUtils.isEmpty(groupID)) {
            if (callback != null) {
...
//                    if (!doingGetAllUserInfo && (!hasGetAllGroupMembers() ||
//                            hasGetPartGroupMembers())) {
                    if (!isNetworkConnected()) {
                        onCallBackFail(callback);
                        return;
                    }
                    try {
                        ...
                        onCallBackFail(callback);
                        NLog.d(TAG, "getGroupMembers occurs HttpException e=" + e.toString() +
                                "mGetAllUserInfoState=" + mGetAllUserInfoState);
                        return;
                    }
//                    } else {
...
//                    }
            ...
            public void run() {
                if (kickedUserIDs != null && kickedUserIDs.size() > 0) {
                    for (String userIDs : kickedUserIDs) {
                        if (mGroupMemberDao != null) {
                            mGroupMemberDao.queryBuilder().where(GroupMemberDao.Properties
                                    .GroupId.eq(groupID), GroupMemberDao.Properties.UserId.eq
                                    (userIDs)).buildDelete()
    ...
    public void deleteGroupMembers(final String groupID) {
        mWorkHandler.post(new Runnable() {
            ...
            public void run() {
                if (mGroupMemberDao != null) {
                    mGroupMemberDao.queryBuilder().where(GroupMemberDao.Properties.GroupId.eq
                            (groupID)).buildDelete().executeDeleteWithoutDetachingEntities();
                }
            }
        });
    }
    ...
    public boolean isFriendsRelationship(String userID) {
        ...
        else return getFriendByID(userID) != null;
    }
    ...
    public Friend getFriendByID(String userID) {
        if (TextUtils.isEmpty(userID)) {
            ...
            if (mFriendDao != null) {
                return mFriendDao.queryBuilder().where(FriendDao.Properties.UserId.eq(userID))
                        .unique();
            } else {
                return null;
            }
        ...
        if (TextUtils.isEmpty(userID)) {
            ...
            mWorkHandler.post(new Runnable() {
                ...
                public void run() {
                    Friend friend = null;
                    if (mFriendDao != null) {
                        friend = mFriendDao.queryBuilder().where(FriendDao.Properties.UserId.eq
                                (userID)).unique();
                    }
                    if (callback != null) {
                        if (friend != null) {
...
//                                sp.edit().putInt("getAllUserInfoState", mGetAllUserInfoState)
//                                        .apply();
//                            } catch (HttpException e) {
//                                onCallBackFail(callback);
//                                NLog.d(TAG, "getGroupsByID occurs HttpException e=" + e.toString
//                                        () + "mGetAllUserInfoState=" + mGetAllUserInfoState);
//                                return;
//                            }
//                        } else {
...
//    }
        ...
        if (result != null) {
            if (requestCode == GET_TOKEN) {
                ...
                if (tokenResponse.getCode() == 200) {
                    ...
                    SharedPreferences sp = mContext.getSharedPreferences("config", Context
                            .MODE_PRIVATE);
    ...
    private List<GroupMember> setCreatedToTop(List<GetGroupMemberResponse.ResultEntity>
        ...
        for (GetGroupMemberResponse.ResultEntity group : groupMember) {
            ...
            GroupMember newMember = new GroupMember(groupID, group.getUser().getId(), group
                    .getUser().getNickname(), Uri.parse(group.getUser().getPortraitUri()), group
                    .getDisplayName(), CharacterParser.getInstance().getSpelling(group.getUser()
                    .getNickname()), CharacterParser.getInstance().getSpelling(group
                    .getDisplayName()), groupName, CharacterParser.getInstance().getSpelling
                    (groupName), groupPortraitUri);
            if (group.getRole() == 0) {
                created = newMember;
            } else {
    ...
    private boolean hasGetAllGroupMembers() {
        return ((mGetAllUserInfoState & GROUPMEMBERS) != 0) && ((mGetAllUserInfoState &
                PARTGROUPMEMBERS) == 0);
    }
...
//            Groups groups = new Groups(groupInfoResponse.getResult().getId(), groupInfoResponse
//                    .getResult().getName(), groupInfoResponse.getResult().getPortraitUri());
...
//    }
    ...
    private String getPortrait(Friend friend) {
        if (friend != null) {
            if (friend.getPortraitUri() == null || TextUtils.isEmpty(friend.getPortraitUri()
                    .toString())) {
                if (TextUtils.isEmpty(friend.getUserId())) {
                    ...
                    if (userInfo != null) {
                        if (userInfo.getPortraitUri() != null && !TextUtils.isEmpty(userInfo
                                .getPortraitUri().toString())) {
                            return userInfo.getPortraitUri().toString();
                        } else {
                    ...
                    List<GroupMember> groupMemberList = getGroupMembersWithUserId(friend
                            .getUserId());
                    if (groupMemberList != null && groupMemberList.size() > 0) {
                        GroupMember groupMember = groupMemberList.get(0);
                        if (groupMember.getPortraitUri() != null && !TextUtils.isEmpty
                                (groupMember.getPortraitUri().toString()))
                            return groupMember.getPortraitUri().toString();
                    }
                    String portrait = RongGenerate.generateDefaultAvatar(friend.getName(), friend
                            .getUserId());
    ...
    private Uri getPortrait(GroupMember groupMember) {
        if (groupMember != null) {
            if (groupMember.getPortraitUri() == null || TextUtils.isEmpty(groupMember
                    .getPortraitUri().toString())) {
                if (TextUtils.isEmpty(groupMember.getUserId())) {
                    ...
                    List<GroupMember> groupMemberList = getGroupMembersWithUserId(groupMember
                            .getUserId());
                    if (groupMemberList != null && groupMemberList.size() > 0) {
                        GroupMember member = groupMemberList.get(0);
                        if (member.getPortraitUri() != null && !TextUtils.isEmpty(member
                                .getPortraitUri().toString())) {
                            return member.getPortraitUri();
                        }
                    }
                    String portrait = RongGenerate.generateDefaultAvatar(groupMember.getName(),
