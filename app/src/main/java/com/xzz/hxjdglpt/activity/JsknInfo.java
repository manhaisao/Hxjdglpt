 ...
 */
@ContentView(R.layout.aty_jskn_info)
public class JsknInfo extends BaseActivity implements ViewPager.OnPageChangeListener {
    ...
    @ViewInject(R.id.jskn_cun)
    private TextView mCun;
    @ViewInject(R.id.jskn_grid)
            ...
            if (!TextUtils.isEmpty(path[i])) {
                ...
                img.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        intent.setClass(JsknInfo.this, ShowImageActivity.class);
...
//        tvTitle.setText(getFile(position));
    ...
    private void loadData() {
        SuccinctProgress.showSuccinctProgress(JsknInfo.this, "数据请求中···", SuccinctProgress
                ...
                try {
                    ...
                    if ("1".equals(resultCode)) {
                        ...
                        Village v = (Village) gson.fromJson(result.getJSONObject("data").toString
                                (), Village.class);
                        mName.setText(jskn.getName());
                        mCun.setText(v.getName());
                        mGrid.setText(gridId);
//                        mCode.setText(jskn.getSfzh());
                        if(isContain()){
                            mCode.setText(jskn.getSfzh());
                        }else if (!TextUtils.isEmpty(jskn.getSfzh()) && jskn.getSfzh().length() > 8) {
                            String sfzh = jskn.getSfzh().substring(0, jskn.getSfzh().length() - 8);
                            mCode.setText(sfzh + "********");
                        }
                        mAddress.setText(jskn.getAddress());
                        mReason.setText(jskn.getReason());
                        mPhone.setText(jskn.getPhone());
                        mBz.setText(jskn.getBz());
                    } else if ("2".equals(resultCode)) {
                        DialogUtil.showTipsDialog(JsknInfo.this);
                    } else if ("3".equals(resultCode)) {
                        ToastUtil.show(JsknInfo.this, R.string.load_fail);
                    }
