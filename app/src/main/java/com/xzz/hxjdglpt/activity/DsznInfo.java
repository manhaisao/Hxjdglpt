 ...
 */
@ContentView(R.layout.aty_dszninfo_info)
public class DsznInfo extends BaseActivity implements ViewPager.OnPageChangeListener {
    ...
    @ViewInject(R.id.dszninfo_d_name)
    private TextView mName;
    @ViewInject(R.id.dszninfo_d_cun)
    private TextView mCun;
    @ViewInject(R.id.dszninfo_d_grid)
    private TextView mGrid;
    @ViewInject(R.id.dszninfo_d_sex)
    private TextView mSex;
    @ViewInject(R.id.dszninfo_d_fname)
    private TextView mFname;
    @ViewInject(R.id.dszninfo_d_fsfzh)
    private TextView mFsfzh;
    @ViewInject(R.id.dszninfo_d_mname)
    private TextView mMname;
    @ViewInject(R.id.dszninfo_d_msfzh)
    private TextView mMsfzh;
    @ViewInject(R.id.dszninfo_d_address)
    private TextView mAddress;
    @ViewInject(R.id.dszninfo_d_phone)
    private TextView mPhone;
    @ViewInject(R.id.dszninfo_d_bz)
            ...
            if (!TextUtils.isEmpty(path[i])) {
                ...
                img.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        intent.setClass(DsznInfo.this, ShowImageActivity.class);
...
//        tvTitle.setText(getFile(position));
    ...
    private void loadData() {
        SuccinctProgress.showSuccinctProgress(DsznInfo.this, "数据请求中···", SuccinctProgress
                ...
                try {
                    ...
                    if ("1".equals(resultCode)) {
                        ...
                        Village v = (Village) gson.fromJson(result.getJSONObject("data").toString
                                (), Village.class);
                        mName.setText(dszn.getZnName());
                        mCun.setText(v.getName());
                        mGrid.setText(gridId);
                        mSex.setText(dszn.getSex());
                        mFname.setText(dszn.getfName());
//                        mFsfzh.setText(dszn.getfSfzh());
                        if(isContain()){
                            mFsfzh.setText(dszn.getfSfzh());
                        }else if (!TextUtils.isEmpty(dszn.getfSfzh()) && dszn.getfSfzh().length() > 8) {
                            String sfzh = dszn.getfSfzh().substring(0, dszn.getfSfzh().length() -
                                    8);
                            mFsfzh.setText(sfzh + "********");
                        }
                        mMname.setText(dszn.getmName());
//                        mMsfzh.setText(dszn.getmSfzh());
                        if(isContain()){
                            mFsfzh.setText(dszn.getfSfzh());
                        }else if (!TextUtils.isEmpty(dszn.getmSfzh()) && dszn.getmSfzh().length() > 8) {
                            String sfzh = dszn.getmSfzh().substring(0, dszn.getmSfzh().length() -
                                    8);
                            mMsfzh.setText(sfzh + "********");
                        }
                        mPhone.setText(dszn.getPhone());
                        mAddress.setText(dszn.getAddress());
                        mBz.setText(dszn.getBz());
                    } else if ("2".equals(resultCode)) {
                        DialogUtil.showTipsDialog(DsznInfo.this);
                    } else if ("3".equals(resultCode)) {
                        ToastUtil.show(DsznInfo.this, R.string.load_fail);
                    }
