...
public class FlfwGzdtDetailActivity extends BaseActivity {
    ...
    public void initData() {
        String label = "创建人：" + yzyh.getCreateUserName() + " \t创建时间：" + yzyh.getCreateTime();
        tvTime.setText(label);
        tvNoTitle.setText(yzyh.getTitle());
        tvContent.setText(yzyh.getContent());
        initFj();
    }

    private boolean isContain() {
        for (Role r : roles) {
            if ("4261".equals(r.getRoleId()) || yzyh.getCreateUser().equals(user.getUserId())) {
                return true;
            }
        }
        return false;
    ...
    private void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.hx_title_right:
                ...
                final Dialog dialog = new Dialog(FlfwGzdtDetailActivity.this);
    ...
    private void delData() {
        SuccinctProgress.showSuccinctProgress(FlfwGzdtDetailActivity.this, "数据提交中···",
                SuccinctProgress.THEME_ULTIMATE, false, true);
        String url = "";
        url = ConstantUtil.BASE_URL + "/lawDt/"+yzyh.getId();
            ...
            public void onMySuccess(JSONObject result) {
                ...
                try {
                    ...
                    if ("0".equals(resultCode)) {
                        ToastUtil.show(FlfwGzdtDetailActivity.this, R.string.del_success);
                        finish();
                    } else if ("2".equals(resultCode)) {
                        DialogUtil.showTipsDialog(FlfwGzdtDetailActivity.this);
                    } else if ("3".equals(resultCode)) {
                        ToastUtil.show(FlfwGzdtDetailActivity.this, R.string.del_fail);
                    }
    ...
    private void initFj() {
        ...
        if (!TextUtils.isEmpty(yzyh.getFilePath())) {
            final String[] path = yzyh.getFilePath().split(",");
            ...
            if (path.length == fileName.length) {
                ...
                for (int i = 0; i < path.length; i++) {
                    if (!TextUtils.isEmpty(fileName[i]) || !TextUtils.isEmpty(path[i])) {
                        ...
                        int pxValue = DensityUtil.dip2px(FlfwGzdtDetailActivity.this, 5);
                        ...
                        tv.setOnClickListener(new View.OnClickListener() {
                            ...
                            public void onClick(View v) {
                                if (fName.endsWith(".jpg") || fName.endsWith(".png") || fName
                                    ...
                                    intent.setClass(FlfwGzdtDetailActivity.this, ShowImageActivity
                                            .class);
                                    startActivity(intent);
                                } else {
                                    File file = new File(ConstantUtil.BASE_PATH + fName);
                                    if (file.exists()) {
                                        Intent intent = new Intent(FlfwGzdtDetailActivity.this,
                                        ...
                                        // .zxtz_detail_tip));
                                        tvCon.setText(FlfwGzdtDetailActivity.this.getText(R.string
                                                .zxtz_detail_content));
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case OkHttpManager.DOWNLOAD_SUCCESS:
                    ToastUtil.show(FlfwGzdtDetailActivity.this, R.string.download_success);
                    times = 0;
                    dialog.dismiss();
                    break;
                case OkHttpManager.DOWNLOAD_FAIL:
                    ToastUtil.show(FlfwGzdtDetailActivity.this, R.string.download_failed);
                    times = 0;
                    dialog.dismiss();
                    break;
                case OkHttpManager.NETWORK_FAILURE:
                    ToastUtil.show(FlfwGzdtDetailActivity.this, R.string.net_unuser);
                    times = 0;
                    dialog.dismiss();
                    break;
                default:
