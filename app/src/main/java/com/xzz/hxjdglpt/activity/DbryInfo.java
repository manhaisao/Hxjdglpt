 ...
 */
@ContentView(R.layout.aty_dbry_info)
public class DbryInfo extends BaseActivity implements ViewPager.OnPageChangeListener {
    ...
    @ViewInject(R.id.db_cun)
    private TextView mCun;
    @ViewInject(R.id.db_grid)
    private TextView mGrid;
    @ViewInject(R.id.db_sfzh)
    private TextView mSfzh;
    @ViewInject(R.id.db_type)
    private TextView mType;
    @ViewInject(R.id.db_address)
    private TextView mAddress;
    @ViewInject(R.id.db_bz)
    private TextView mBz;
    @ViewInject(R.id.db_dbzh)
    private TextView mDbzh;
    @ViewInject(R.id.db_dbs)
    private TextView mDbs;
    @ViewInject(R.id.db_wgy)
    private TextView mWgy;
    @ViewInject(R.id.db_wgyphone)
    private TextView mWgyphoe;
    @ViewInject(R.id.db_reason)
    ...
    public void initView() {
        ...
        if (dbry.getPicpath() == null || TextUtils.isEmpty(dbry.getPicpath())) {
...
//                        intent.putExtra("isLocal", false);
                        ...
                        intent.setClass(DbryInfo.this, ShowImageActivity.class);
...
//        tvTitle.setText(getFile(position));
    ...
    private void loadData() {
        SuccinctProgress.showSuccinctProgress(DbryInfo.this, "数据请求中···", SuccinctProgress
                ...
                try {
                    ...
                    if ("1".equals(resultCode)) {
                        ...
                        Village v = (Village) gson.fromJson(result.getJSONObject("data").toString
                                (), Village.class);
                        mName.setText(dbry.getName());
                        mCun.setText(v.getName());
                        mGrid.setText(gridId);
                        if (isContain()) {
                            mSfzh.setText(dbry.getSfzh());
                        } else if (!TextUtils.isEmpty(dbry.getSfzh()) && dbry.getSfzh().length() > 8) {
                            String sfzh = dbry.getSfzh().substring(0, dbry.getSfzh().length() - 8);
                            mSfzh.setText(sfzh + "********");
                        }
                        String[] types = getResources().getStringArray(R.array.dbtype);
                        if ("0".equals(dbry.getType())) {
                            mType.setText(types[0]);
                        } else if ("1".equals(dbry.getType())) {
                            mType.setText(types[1]);
                        }
                        mAddress.setText(dbry.getAddress());
                        mBz.setText(dbry.getBz());
                        mDbs.setText(dbry.getpNum());
                        mDbzh.setText(dbry.getDbCode());
                        mWgy.setText(dbry.getWgy());
                        mWgyphoe.setText(dbry.getWgyphone());
                        mReason.setText(dbry.getReason());
                    } else if ("2".equals(resultCode)) {
                        DialogUtil.showTipsDialog(DbryInfo.this);
                    } else if ("3".equals(resultCode)) {
                        ToastUtil.show(DbryInfo.this, R.string.load_fail);
                    }
