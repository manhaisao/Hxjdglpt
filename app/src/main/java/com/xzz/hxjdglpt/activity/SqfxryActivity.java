 ...
 */
@ContentView(R.layout.aty_sqfxry)
public class SqfxryActivity extends BaseActivity {
    ...
    private EditText edtZm;//罪名
    @ViewInject(R.id.jsbhz_jhrgx)
    private EditText edtJzlb;//矫正类别
    @ViewInject(R.id.jsbhz_jhrphone)
    private EditText edtXqxz;//原判刑期及刑种
    @ViewInject(R.id.jsbhz_stime)
    private TextView tvStime;//开始时间
    @ViewInject(R.id.jsbhz_etime)
    private TextView tvEtime;//结束时间

    @ViewInject(R.id.jsbhz_gkxz_add)
    private TextView mAdd;//矫正小组新增
    @ViewInject(R.id.jsbhz_t_gkxz_lay)
    private RelativeLayout gkxzLay;//矫正小组
    ...
    public void initData() {
        if (!isAdd) {
            ...
            edtName.setText(sqfxry.getName());//姓名
            edtAddress.setText(sqfxry.getAddress());//地址
            edtCode.setText(sqfxry.getSfzh());
            tvEtime.setText(sqfxry.getJzETime());
            edtPhone.setText(sqfxry.getPhone());
            tvStime.setText(sqfxry.getJzSTime());
            edtJzlb.setText(sqfxry.getJzType());
            edtXqxz.setText(sqfxry.getXqxz());
            edtZm.setText(sqfxry.getZm());
            getXzCount();
        } else {
            ...
            public void onMySuccess(JSONObject result) {
                ...
                try {
                    ...
                    if ("1".equals(resultCode)) {
                        ...
                        DialogUtil.showTipsDialog(SqfxryActivity.this);
                    } else if ("3".equals(resultCode)) {
                        ToastUtil.show(SqfxryActivity.this, R.string.load_fail);
                    }
     ...
     */
    private void commit() {
        String url = ConstantUtil.BASE_URL + "/sqfxry/commitSqfxry";
        ...
        if (!TextUtils.isEmpty(tvEtime.getText().toString())) {
            params.put("jzETime", tvEtime.getText().toString());
        } else {
            params.put("jzETime", "");
        }
        ...
        if (!TextUtils.isEmpty(tvStime.getText().toString())) {
            params.put("jzSTime", tvStime.getText().toString());
        } else {
            params.put("jzSTime", "");
        }
        if (!TextUtils.isEmpty(edtXqxz.getText().toString())) {
            params.put("xqxz", edtXqxz.getText().toString());
        } else {
            params.put("xqxz", "");
        }
        if (!TextUtils.isEmpty(edtZm.getText().toString())) {
            params.put("zm", edtZm.getText().toString());
        } else {
            params.put("zm", "");
        }
        if (!TextUtils.isEmpty(edtJzlb.getText().toString())) {
            params.put("jzType", edtJzlb.getText().toString());
        } else {
            params.put("jzType", "");
        }
        params.put("bz", "");
        SuccinctProgress.showSuccinctProgress(this, "数据提交中···", SuccinctProgress.THEME_ULTIMATE,
                false, true);
        VolleyRequest.RequestPost(this, url, "commitSqfxry", params, new VolleyListenerInterface
                (this, VolleyListenerInterface.mListener, VolleyListenerInterface.mErrorListener) {
            ...
            public void onMySuccess(JSONObject result) {
                ...
                try {
                    String resultCode = result.getString("resultCode");
                    if ("1".equals(resultCode)) {
                        ToastUtil.show(SqfxryActivity.this, R.string.commit_success);
                        finish();
                    } else if ("2".equals(resultCode)) {
                        DialogUtil.showTipsDialog(SqfxryActivity.this);
                    } else if ("3".equals(resultCode)) {
                        ToastUtil.show(SqfxryActivity.this, R.string.commit_fail);
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
                }
    ...
    @Event(value = {R.id.jsbhz_commit, R.id.iv_back, R.id.jsbhz_gkxz_add, R.id.jsbhz_stime, R.id
            .jsbhz_etime}, type = View.OnClickListener.class)
    private void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.jsbhz_gkxz_add:
                ...
                intent.setClass(SqfxryActivity.this, GkxzListActivity.class);
                intent.putExtra("type", "5");
                intent.putExtra("pId", String.valueOf(sqfxry.getId()));
                startActivity(intent);
                break;
            case R.id.jsbhz_stime:
                showBottoPopupWindow(tvStime);
                break;
            case R.id.jsbhz_etime:
                showBottoPopupWindow(tvEtime);
                break;
        }
    }
    ...
    public void onStop() {
        super.onStop();
        BaseApplication.getRequestQueue().cancelAll("commitSqfxry");
        BaseApplication.getRequestQueue().cancelAll("updateSqfxry");
        BaseApplication.getRequestQueue().cancelAll("getCount");
    }


    private void update() {
        String url = ConstantUtil.BASE_URL + "/sqfxry/updateSqfxry";
        Map<String, String> params = new HashMap<String, String>();
        params.put("userId", user.getUserId());
        params.put("token", user.getToken());
        params.put("id", String.valueOf(sqfxry.getId()));
        params.put("gridId", gridId);
        if (!TextUtils.isEmpty(edtName.getText().toString())) {
            params.put("name", edtName.getText().toString());
        } else {
        ...
        SuccinctProgress.showSuccinctProgress(this, "数据修改中···", SuccinctProgress.THEME_ULTIMATE,
                false, true);
        VolleyRequest.RequestPost(this, url, "updateSqfxry", params, new VolleyListenerInterface
                (this, VolleyListenerInterface.mListener, VolleyListenerInterface.mErrorListener) {
            ...
            public void onMySuccess(JSONObject result) {
                ...
                try {
                    String resultCode = result.getString("resultCode");
                    if ("1".equals(resultCode)) {
                        ToastUtil.show(SqfxryActivity.this, R.string.modify_success);
                        finish();
                    } else if ("2".equals(resultCode)) {
                        DialogUtil.showTipsDialog(SqfxryActivity.this);
                    } else if ("3".equals(resultCode)) {
                        ToastUtil.show(SqfxryActivity.this, R.string.modify_fail);
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
                }
