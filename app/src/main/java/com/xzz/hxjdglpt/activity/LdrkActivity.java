 ...
 */
@ContentView(R.layout.aty_ldrk)
public class LdrkActivity extends BaseActivity implements View.OnTouchListener {
    ...
    @ViewInject(R.id.ldrk_now_address)
    private EditText edtNowAddress;//现住址

    @ViewInject(R.id.ldrk_ldlx)
        ...
        gridView1.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View v, int position, long id) {
                if (position == 0 && imageItem.size() == ConstantUtil.ALL_IMAGE_COUNT) { //第一张为默认图片
                    ToastUtil.show(LdrkActivity.this, "图片数已至上限");
    ...
    public boolean onTouch(View view, MotionEvent motionEvent) {
        ...
        if ((view.getId() == R.id.ldrk_bz && canVerticalScroll(edtBz))) {
    ...
    public void initData() {
        if (!isAdd) {
            ...
            for (int i = 0; i < ldType.length; i++) {
                if (ldType[i].equals(ldrk.getLdlx())) {
                    spldlx.setSelection(i);
                    break;
                }
            }
            edtName.setText(ldrk.getName());//姓名
            edtAddress.setText(ldrk.getAddress());//地址
            edtCode.setText(ldrk.getSfzh());
            edtNowAddress.setText(ldrk.getNowAddress());
            edtBz.setText(ldrk.getBz());//备注
        } else {
     ...
     */
    private void commit() {
        String url = ConstantUtil.BASE_URL + "/ldrk/commitLdrk";
        ...
        if (!TextUtils.isEmpty(edtNowAddress.getText().toString())) {
            params.put("nowAddress", edtNowAddress.getText().toString());
        } else {
            params.put("nowAddress", "");
        }
        ...
        SuccinctProgress.showSuccinctProgress(this, "数据提交中···", SuccinctProgress.THEME_ULTIMATE,
                false, true);
        VolleyRequest.RequestPost(this, url, "commitLdrk", params, new VolleyListenerInterface
            ...
            public void onMySuccess(JSONObject result) {
                ...
                try {
                    ...
                    if ("1".equals(resultCode)) {
                        ToastUtil.show(LdrkActivity.this, R.string.commit_success);
                        finish();
                    } else if ("2".equals(resultCode)) {
                        DialogUtil.showTipsDialog(LdrkActivity.this);
                    } else if ("3".equals(resultCode)) {
                        ToastUtil.show(LdrkActivity.this, R.string.commit_fail);
                    }
    ...
    @Event(value = {R.id.ldrk_commit, R.id.iv_back}, type = View.OnClickListener.class)
    private void onClick(View v) {
        switch (v.getId()) {
            case R.id.ldrk_commit:
                if (isAdd) {
                    commit();
                } else {
                    update();
    ...
    public void onStop() {
        super.onStop();
        BaseApplication.getRequestQueue().cancelAll("commitLdrk");
        BaseApplication.getRequestQueue().cancelAll("updateLdrk");
    }


    private void update() {
        String url = ConstantUtil.BASE_URL + "/ldrk/updateLdrk";
        Map<String, String> params = new HashMap<String, String>();
        params.put("userId", user.getUserId());
        params.put("token", user.getToken());
        params.put("id", String.valueOf(ldrk.getId()));
        params.put("gId", gridId);
        if (!TextUtils.isEmpty(edtName.getText().toString())) {
            params.put("name", edtName.getText().toString());
        } else {
        ...
        SuccinctProgress.showSuccinctProgress(this, "数据修改中···", SuccinctProgress.THEME_ULTIMATE,
                false, true);
        VolleyRequest.RequestPost(this, url, "updateLdrk", params, new VolleyListenerInterface
                (this, VolleyListenerInterface.mListener, VolleyListenerInterface.mErrorListener) {
            ...
            public void onMySuccess(JSONObject result) {
                ...
                try {
                    String resultCode = result.getString("resultCode");
                    if ("1".equals(resultCode)) {
                        ToastUtil.show(LdrkActivity.this, R.string.modify_success);
                        finish();
                    } else if ("2".equals(resultCode)) {
                        DialogUtil.showTipsDialog(LdrkActivity.this);
                    } else if ("3".equals(resultCode)) {
                        ToastUtil.show(LdrkActivity.this, R.string.modify_fail);
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
                }
