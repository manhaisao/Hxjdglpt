 ...
 */
@ContentView(R.layout.aty_zdqsn_info)
public class ZdqsnInfo extends BaseActivity {
            ...
            public void onMySuccess(JSONObject result) {
                ...
                try {
                    ...
                    if ("1".equals(resultCode)) {
                        ...
                        DialogUtil.showTipsDialog(ZdqsnInfo.this);
                    } else if ("3".equals(resultCode)) {
                        ToastUtil.show(ZdqsnInfo.this, R.string.load_fail);
                    }
    ...
    private void loadData() {
        SuccinctProgress.showSuccinctProgress(ZdqsnInfo.this, "数据请求中···", SuccinctProgress
            ...
            public void onMySuccess(JSONObject result) {
                ...
                try {
                    ...
                    if ("1".equals(resultCode)) {
                        ...
                        if (isContain()) {
                            mName.setText(zdqsn.getName());
                            mSfzh.setText(zdqsn.getSfzh());
                            mPhone.setText(zdqsn.getPhone());
                            mJhr.setText(zdqsn.getJhr());
                            mJhrDh.setText(zdqsn.getJhrPhone());
                        } else {
                            if (!TextUtils.isEmpty(zdqsn.getSfzh()) && zdqsn.getSfzh().length() >
                                    8) {
                                String sfzh = zdqsn.getSfzh().substring(0, zdqsn.getSfzh().length
                                        () - 8);
                                mSfzh.setText(sfzh + "********");
                            }
                            if (zdqsn.getName() != null && zdqsn.getName().length() > 1) {
                                String name = zdqsn.getName().substring(0, 1);
                                mName.setText(name + "**");
                            }
                            if (!TextUtils.isEmpty(zdqsn.getPhone()) && zdqsn.getPhone().length()
                                    > 4) {
                                String phone = zdqsn.getPhone().substring(0, zdqsn.getPhone()
                                        .length() - 4);
                                mPhone.setText(phone + "****");
                            }
                            if (zdqsn.getJhr() != null && zdqsn.getJhr().length() > 1) {
                                String name = zdqsn.getJhr().substring(0, 1);
                                mJhr.setText(name + "**");
                            }
                            if (!TextUtils.isEmpty(zdqsn.getJhrPhone()) && zdqsn.getJhrPhone()
                                    .length() > 4) {
                                String phone = zdqsn.getJhrPhone().substring(0, zdqsn.getJhrPhone
                                        ().length() - 4);
                                mJhrDh.setText(phone + "****");
                            }
                        }
                        mAddress.setText(zdqsn.getAddress());
                        switch (zdqsn.getRelationType()) {
                            case 1:
    ...
    private void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.jsbhz_info_btn:
                intent.setClass(ZdqsnInfo.this, ZdryGzrzActivity.class);
            ...
            case R.id.jsbhz_d_xz:
                if (!("index".equals(isFrom))) {
                    intent.setClass(ZdqsnInfo.this, GkxzListActivity.class);
                    intent.putExtra("type", "3");
                    intent.putExtra("pId", String.valueOf(zdqsn.getId()));
                    startActivity(intent);
                }
