...
public class JzgyActivity extends BaseActivity implements View.OnFocusChangeListener, View
        .OnTouchListener {
    ...
    public void initData() {
        if (!isAdd) {
            ...
            jzgy = getIntent().getParcelableExtra("jzgy");
            for (int i = 0; i < hlType.length; i++) {
                if (hlType[i].equals(jzgy.getDengji())) {
                    spDj.setSelection(i);
                    break;
                }
            }
            edtName.setText(jzgy.getName());//姓名
//            if(isContain()){
                edtSfzh.setText(jzgy.getNo());
//            }else if (!TextUtils.isEmpty(jzgy.getNo()) && jzgy.getNo().length() > 8) {
//                String sfzh = jzgy.getNo().substring(0, jzgy.getNo().length() - 8);
//                edtSfzh.setText(sfzh+"********");
//            }
            edtCj.setText(jzgy.getCunju());
            edtBz.setText(jzgy.getBz());//备注
        } else {
    ...
    private void commit() {
        SuccinctProgress.showSuccinctProgress(this, "数据提交中···", SuccinctProgress.THEME_ULTIMATE,
                false, true);
        String url = ConstantUtil.BASE_URL + "/jzgy/commitJzgy";
        ...
        VolleyRequest.RequestPost(this, url, "commitJzgy", params, new VolleyListenerInterface
            ...
            public void onMySuccess(JSONObject result) {
                ...
                try {
                    ...
                    if ("1".equals(resultCode)) {
                        ToastUtil.show(JzgyActivity.this, R.string.commit_success);
                        finish();
                    } else if ("2".equals(resultCode)) {
                        DialogUtil.showTipsDialog(JzgyActivity.this);
                    } else if ("3".equals(resultCode)) {
                        ToastUtil.show(JzgyActivity.this, R.string.commit_fail);
                    }
    ...
    public void onStop() {
        super.onStop();
        BaseApplication.getRequestQueue().cancelAll("commitJzgy");
        BaseApplication.getRequestQueue().cancelAll("updateJzgy");
    }
    ...
    private void update() {
        SuccinctProgress.showSuccinctProgress(this, "数据修改中···", SuccinctProgress.THEME_ULTIMATE,
                false, true);
        String url = ConstantUtil.BASE_URL + "/jzgy/updateJzgy";
        Map<String, String> params = new HashMap<String, String>();
        params.put("userId", user.getUserId());
        params.put("token", user.getToken());
        params.put("id", String.valueOf(jzgy.getId()));
        if (!TextUtils.isEmpty(edtName.getText().toString())) {
            params.put("name", edtName.getText().toString());
        } else {
            params.put("name", "");
        ...
        VolleyRequest.RequestPost(this, url, "updateJzgy", params, new VolleyListenerInterface
                (this, VolleyListenerInterface.mListener, VolleyListenerInterface.mErrorListener) {
            ...
            public void onMySuccess(JSONObject result) {
                ...
                try {
                    String resultCode = result.getString("resultCode");
                    if ("1".equals(resultCode)) {
                        ToastUtil.show(JzgyActivity.this, R.string.modify_success);
                        finish();
                    } else if ("2".equals(resultCode)) {
                        DialogUtil.showTipsDialog(JzgyActivity.this);
                    } else if ("3".equals(resultCode)) {
                        ToastUtil.show(JzgyActivity.this, R.string.modify_fail);
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
                }
