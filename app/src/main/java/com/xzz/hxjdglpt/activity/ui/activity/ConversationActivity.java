...
import com.xzz.hxjdglpt.activity.ui.fragment.ConversationFragmentEx;
import com.xzz.hxjdglpt.activity.ui.widget.LoadingDialog;
...
//                        setTitle(TextTypingTitle);
                        textView.setText(TextTypingTitle);
                        break;
                    case SET_VOICE_TYPING_TITLE:
//                        setTitle(VoiceTypingTitle);
                        textView.setText(VoiceTypingTitle);
                        break;
        ...
        RongIMClient.setTypingStatusListener(new RongIMClient.TypingStatusListener() {
            @Override
            public void onTypingStatusChanged(Conversation.ConversationType type, String
                    targetId, Collection<TypingStatus> typingStatusSet) {
...
//            @Override
//            public ArrayList<String> getMemberList(String groupId, final RongCallKit
//                    .OnGroupMembersResult result) {
...
//        });
    ...
    private void isPushMessage(Intent intent) {
        ...
        if (intent == null || intent.getData() == null) return;
        //push
        if (intent.getData().getScheme().equals("rong") && intent.getData().getQueryParameter
                ("isFromPush") != null) {
            ...
            if (intent.getData().getQueryParameter("isFromPush").equals("true")) {
                ...
                isFromPush = true;
                enterActivity();
            } else if (RongIM.getInstance().getCurrentConnectionStatus().equals(RongIMClient
                    .ConnectionStatusListener.ConnectionStatus.DISCONNECTED)) {
                if (mDialog != null && !mDialog.isShowing()) {
                    mDialog.show();
                }
                if (intent.getData().getPath().contains("conversation/system")) {
                    Intent intent1 = new Intent(this, MainMIActivity.class);
     ...
     * <p>
     * 作用：由于在 manifest 中 intent-filter 是配置在 ConversationActivity 下面，所以收到消息后点击notifacition 会跳转到
     * DemoActivity。
     * 以跳到 MainActivity 为例：
    ...
    private void enterFragment(Conversation.ConversationType mConversationType, String mTargetId) {
        ...
        Uri uri = Uri.parse("rong://" + getApplicationInfo().packageName).buildUpon().appendPath
                ("conversation").appendPath(mConversationType.getName().toLowerCase())
     ...
     */
    private void setActionBarTitle(Conversation.ConversationType conversationType, String
            targetId) {
...
//            setTitle(R.string.de_actionbar_system);
            textView.setText(getText(R.string.de_actionbar_system));
...
//            setTitle(R.string.main_customer);
            textView.setText(getText(R.string.main_customer));
        } else {
//            setTitle(R.string.de_actionbar_sub_defult);
            textView.setText(getText(R.string.de_actionbar_sub_defult));
        }
...
//            setTitle(targetId);
    ...
    private void setAppPublicServiceActionBar(String targetId) {
        ...
        RongIM.getInstance().getPublicServiceProfile(Conversation.PublicServiceType
                .APP_PUBLIC_SERVICE, targetId, new RongIMClient
                .ResultCallback<PublicServiceProfile>() {
            @Override
            public void onSuccess(PublicServiceProfile publicServiceProfile) {
//                setTitle(publicServiceProfile.getName());
                textView.setText(publicServiceProfile.getName());
            }
    ...
    private void setPublicServiceActionBar(String targetId) {
        ...
        RongIM.getInstance().getPublicServiceProfile(Conversation.PublicServiceType
                .PUBLIC_SERVICE, targetId, new RongIMClient.ResultCallback<PublicServiceProfile>() {
            @Override
            public void onSuccess(PublicServiceProfile publicServiceProfile) {
//                setTitle(publicServiceProfile.getName());
                textView.setText(publicServiceProfile.getName());
    ...
    private void setDiscussionActionBar(String targetId) {
        ...
        if (targetId != null) {
            ...
            RongIM.getInstance().getDiscussion(targetId, new RongIMClient
                    .ResultCallback<Discussion>() {
                @Override
    ...
    private void enterSettingActivity() {
        ...
        if (mConversationType == Conversation.ConversationType.PUBLIC_SERVICE ||
            ...
            if (TextUtils.isEmpty(mTargetId)) {
                NToast.shortToast(this, "讨论组尚未创建成功");
            }
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if (KeyEvent.KEYCODE_BACK == event.getKeyCode()) {
            if (fragment != null && !fragment.onBackPressed()) {
                if (isFromPush) {
                    isFromPush = false;
                    startActivity(new Intent(this, MainMIActivity.class));
                    SealAppContext.getInstance().popAllActivity();
                } else {
                    ...
                    if (mConversationType.equals(Conversation.ConversationType.CHATROOM) ||
                            mConversationType.equals(Conversation.ConversationType
                                    .CUSTOMER_SERVICE)) {
                        SealAppContext.getInstance().popActivity(this);
                    } else {
                        SealAppContext.getInstance().popActivity(this);
...
//        SealUserInfoManager.getInstance().getGroupMembers(mTargetId, new SealUserInfoManager
