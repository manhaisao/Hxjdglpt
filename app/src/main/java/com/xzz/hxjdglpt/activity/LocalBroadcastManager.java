...
public final class LocalBroadcastManager {
    ...
    private final HashMap<String, ArrayList<ReceiverRecord>> mActions = new HashMap<String,
    ...
    public boolean sendBroadcast(Intent intent) {
        ...
        synchronized (this.mReceivers) {
            ...
            if (debug) {
                LogUtil.i("Resolving type " + type + " scheme " + scheme + " of intent " + intent);
            }

            ArrayList entries = (ArrayList) this.mActions.get(intent.getAction());
            if (entries != null) {
                if (debug) {
                    LogUtil.i("Action list: " + entries);
                }
                ...
                for (i = 0; i < entries.size(); ++i) {
                    ...
                    if (debug) {
                        LogUtil.i("Matching against filter " + receiver.filter);
                    }

                    if (receiver.broadcasting) {
                        if (debug) {
                            LogUtil.i("  Filter\'s target already added");
                        }
                    } else {
                        ...
                        if (match >= 0) {
                            if (debug) {
                                LogUtil.i("  Filter matched!  match=0x" + Integer.toHexString
                                        (match));
                            }
                            ...
                            LogUtil.i(" Filter did not match: " + reason);
                        }
                    }
                }
        ...
        while (true) {
            ...
            for (int var6 = 0; var6 < brs.length; ++var6) {
                ...
                for (int j = 0; j < var7.receivers.size(); ++j) {
                    ((ReceiverRecord) var7.receivers.get(j)).receiver.onReceive(this.mAppContext,
                            var7.intent);
                }
            }
        }
