...
public class GzdtDetailActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (type == -1) {
            plotGzdt = getIntent().getParcelableExtra("gzdt");
        } else {
            gzdt = getIntent().getParcelableExtra("gzdt");
        }
        initView();
        initData();
        createDialog();
    ...
    public void initData() {
        if (type == -1) {
            String label = "创建人：" + plotGzdt.getFbrName() + " \t创建时间：" + plotGzdt.getFbsj();
            tvTime.setText(label);
            tvNoTitle.setText(plotGzdt.getTitle());
            tvContent.setText(plotGzdt.getContent());
            initPlotFj();
        } else {
    ...
    private boolean isContain() {
        if (type == -1) {
            for (Role r : roles) {
                if ("4261".equals(r.getRoleId()) || plotGzdt.getFbr() == Long.valueOf(user.getUserId())) {
                    return true;
                }
            }
        } else {
    ...
    private void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.hx_title_right:
                ...
                final Dialog dialog = new Dialog(GzdtDetailActivity.this);
    ...
    private void delData() {
        SuccinctProgress.showSuccinctProgress(GzdtDetailActivity.this, "数据提交中···",
                SuccinctProgress.THEME_ULTIMATE, false, true);
        String url = "";
        HashMap<String, String> params = new HashMap<>();
        if (type == -1) {
            url = ConstantUtil.BASE_URL + "/plotGzdt/deletePlotGzdt";
            params.put("id", String.valueOf(plotGzdt.getId()));
        } else {
            url = ConstantUtil.BASE_URL + "/gzdt/deleteGzdt";
            ...
            public void onMySuccess(JSONObject result) {
                ...
                try {
                    ...
                    if ("1".equals(resultCode)) {
                        ToastUtil.show(GzdtDetailActivity.this, R.string.del_success);
                        finish();
                    } else if ("2".equals(resultCode)) {
                        DialogUtil.showTipsDialog(GzdtDetailActivity.this);
                    } else if ("3".equals(resultCode)) {
                        ToastUtil.show(GzdtDetailActivity.this, R.string.del_fail);
                    }
            ...
            if (path.length == fileName.length) {
                ...
                for (int i = 0; i < path.length; i++) {
                    if (!TextUtils.isEmpty(fileName[i]) || !TextUtils.isEmpty(path[i])) {
                        ...
                        int pxValue = DensityUtil.dip2px(GzdtDetailActivity.this, 5);
                        ...
                        tv.setOnClickListener(new View.OnClickListener() {
                            ...
                            public void onClick(View v) {
                                if (fName.endsWith(".jpg") || fName.endsWith(".png") || fName
                                    ...
                                    intent.setClass(GzdtDetailActivity.this, ShowImageActivity
                                            .class);
                                    startActivity(intent);
                                } else {
                                    File file = new File(ConstantUtil.BASE_PATH + fName);
                                    if (file.exists()) {
                                        Intent intent = new Intent(GzdtDetailActivity.this,
                                        ...
                                        // .zxtz_detail_tip));
                                        tvCon.setText(GzdtDetailActivity.this.getText(R.string
                                                .zxtz_detail_content));
     ...
     * 动态添加附件
     */
    private void initPlotFj() {
        lay_fj.removeAllViews();
        if (!TextUtils.isEmpty(plotGzdt.getFilepath())) {
            final String[] path = plotGzdt.getFilepath().split(",");
            String[] fileName = plotGzdt.getFilename().split(",");
            if (path.length == fileName.length) {
                //去除非图片文件路径
                List<String> list = new ArrayList<>();
                for (int i = 0; i < path.length; i++) {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case OkHttpManager.DOWNLOAD_SUCCESS:
                    ToastUtil.show(GzdtDetailActivity.this, R.string.download_success);
                    times = 0;
                    dialog.dismiss();
                    break;
                case OkHttpManager.DOWNLOAD_FAIL:
                    ToastUtil.show(GzdtDetailActivity.this, R.string.download_failed);
                    times = 0;
                    dialog.dismiss();
                    break;
                case OkHttpManager.NETWORK_FAILURE:
                    ToastUtil.show(GzdtDetailActivity.this, R.string.net_unuser);
                    times = 0;
                    dialog.dismiss();
                    break;
                default:
