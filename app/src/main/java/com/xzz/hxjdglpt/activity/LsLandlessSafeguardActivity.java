 ...
 * 被征地农民保障服务
 */
@ContentView(R.layout.activity_ls_landless_safeguard)
public class LsLandlessSafeguardActivity  extends BaseActivity{
    ...
    public void initView() {
        super.initView();
        hXtitle.setText(R.string.ls_cx_05);
    }
    ...
    private void loadData() {
        SuccinctProgress.showSuccinctProgress(LsLandlessSafeguardActivity.this, "数据请求中···", SuccinctProgress
                .THEME_ULTIMATE, false, true);
        String url = ConstantUtil.BASE_URL + "/bzfw/queryByPage?pageIndex="+String.valueOf(pageIndex)+"&pageSize="+String.valueOf(ConstantUtil.PAGE_SIZE);
        VolleyRequest.RequestGet(this, url, "queryBzfw", new VolleyListenerInterface
                ...
                try {
                    ...
                    if ("1".equals(resultCode)) {
                        ...
                        if(jsonArray.length()>0){
                            ...
                            Gson gson = new Gson();
                            List<Baozhang> objs = gson.fromJson(jbrArr.toString(), new
                                    TypeToken<List<Baozhang>>() {
                                    }.getType());
                            StringBuffer sb  = new StringBuffer();
                            for (Baozhang bz:objs){
                                if(sb == null || sb.toString().equals("")){
                                    sb.append(bz.getJbr()+"  "+bz.getJbrphone());
                                }else{
                                    sb.append("\n\n"+bz.getJbr()+"  "+bz.getJbrphone());
                                }
                            }
                            tvBmjbr.setText(sb.toString());
                        }
                    } else if ("2".equals(resultCode)) {
                        DialogUtil.showTipsDialog(LsLandlessSafeguardActivity.this);
                    } else if ("3".equals(resultCode)) {
                        ToastUtil.show(LsLandlessSafeguardActivity.this, R.string.load_fail);
                    }
    ...
    public void onStop() {
        super.onStop();
        BaseApplication.getRequestQueue().cancelAll("queryBzfw");
    }
