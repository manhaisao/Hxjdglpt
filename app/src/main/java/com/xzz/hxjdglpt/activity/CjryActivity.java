 ...
 */
@ContentView(R.layout.aty_cjry)
public class CjryActivity extends BaseActivity implements View.OnFocusChangeListener, View
        .OnTouchListener {
    ...
    @ViewInject(R.id.cjry_cjlx)
    private AppCompatSpinner spCjlx;

    @ViewInject(R.id.cjry_commit)
    private Button btnCommit;

    @ViewInject(R.id.cjry_name)
    private EditText edtName;//姓名

//    @ViewInject(R.id.cjry_birth)
//    private EditText edtBirth;//出生日月

    @ViewInject(R.id.cjry_cjzh)
    private EditText edtCjzh;//残疾证号

//    @ViewInject(R.id.cjry_address)
//    private EditText edtAddress;//住址

    @ViewInject(R.id.cjry_bz)
    ...
    public boolean onTouch(View view, MotionEvent motionEvent) {
        ...
        if ((view.getId() == R.id.cjry_bz && canVerticalScroll(edtBz))) {
    ...
    public void initView() {
        tvTitle.setText("残疾人员信息");
        cjType = getResources().getStringArray(R.array.cj_type_list);
        //适配器
        ...
        //加载适配器
        spCjlx.setAdapter(cjTypeAdpate);
        /*
         * 载入默认图片添加图片加号
         * 通过适配器实现
        ...
        gridView1.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View v, int position, long id) {
                if (position == 0 && imageItem.size() == ConstantUtil.ALL_IMAGE_COUNT) { //第一张为默认图片
                    ToastUtil.show(CjryActivity.this, "图片数已至上限");
    ...
    public void initData() {
        if (!isAdd) {
            ...
            cjry = getIntent().getParcelableExtra("cjry");
            for (int i = 0; i < cjType.length; i++) {
                if (cjType[i].equals(cjry.getCjType())) {
                    spCjlx.setSelection(i);
                    break;
                }
            }
            edtName.setText(cjry.getName());//姓名
            edtCjzh.setText(cjry.getCjCode());//残疾证号
            edtBz.setText(cjry.getBz());//备注
        } else {
    ...
    private void commit() {
        SuccinctProgress.showSuccinctProgress(this, "数据提交中···", SuccinctProgress.THEME_ULTIMATE,
                false, true);
        String url = ConstantUtil.BASE_URL + "/cjry/commitCjry";
        ...
        if (!TextUtils.isEmpty(edtCjzh.getText().toString())) {
            params.put("cjzh", edtCjzh.getText().toString());
        } else {
            params.put("cjzh", "");
        }
        params.put("cjlx", spCjlx.getSelectedItem().toString());
...
//            }
        ...
        VolleyRequest.RequestPost(this, url, "cjry_commit", params, new VolleyListenerInterface
            ...
            public void onMySuccess(JSONObject result) {
                ...
                try {
                    ...
                    if ("1".equals(resultCode)) {
                        ToastUtil.show(CjryActivity.this, R.string.commit_success);
                        finish();
                    } else if ("2".equals(resultCode)) {
                        DialogUtil.showTipsDialog(CjryActivity.this);
                    } else if ("3".equals(resultCode)) {
                        ToastUtil.show(CjryActivity.this, R.string.commit_fail);
                    }
    ...
    @Event(value = {R.id.cjry_commit, R.id.iv_back}, type = View.OnClickListener.class)
    ...
    public void onStop() {
        ...
        BaseApplication.getRequestQueue().cancelAll("cjry_commit");
        BaseApplication.getRequestQueue().cancelAll("updateCjry");
    }
    ...
    private void update() {
        SuccinctProgress.showSuccinctProgress(this, "数据修改中···", SuccinctProgress.THEME_ULTIMATE,
                false, true);
        String url = ConstantUtil.BASE_URL + "/cjry/updateCjry";
        Map<String, String> params = new HashMap<String, String>();
        params.put("userId", user.getUserId());
        params.put("token", user.getToken());
        params.put("id", cjry.getId());
        if (!TextUtils.isEmpty(edtName.getText().toString())) {
            params.put("name", edtName.getText().toString());
        } else {
            params.put("name", "");
        ...
        params.put("picpath", sb.toString());
        VolleyRequest.RequestPost(this, url, "updateCjry", params, new VolleyListenerInterface
                (this, VolleyListenerInterface.mListener, VolleyListenerInterface.mErrorListener) {
            ...
            public void onMySuccess(JSONObject result) {
                ...
                try {
                    String resultCode = result.getString("resultCode");
                    if ("1".equals(resultCode)) {
                        ToastUtil.show(CjryActivity.this, R.string.modify_success);
                        finish();
                    } else if ("2".equals(resultCode)) {
                        DialogUtil.showTipsDialog(CjryActivity.this);
                    } else if ("3".equals(resultCode)) {
                        ToastUtil.show(CjryActivity.this, R.string.modify_fail);
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
                }
