...
import com.xzz.hxjdglpt.activity.ui.adapter.NewFriendListAdapter;
...
public class NewFriendListActivity extends BaseActivity implements NewFriendListAdapter
        .OnItemButtonClick, View.OnClickListener {
...
//        rightButton.setBackgroundDrawable(getResources().getDrawable(R.mipmap
//                .de_address_new_friend));
...
//                    }
//
//                    Collections.sort(userRelationshipResponse.getResult(), new
//                            Comparator<UserRelationshipResponse.ResultEntity>() {
//
//                        @Override
//                        public int compare(UserRelationshipResponse.ResultEntity lhs,
//                                           UserRelationshipResponse.ResultEntity rhs) {
...
//                        UserRelationshipResponse.ResultEntity bean = userRelationshipResponse
//                                .getResult().get(index);
//                        SealUserInfoManager.getInstance().addFriend(new Friend(bean.getUser()
//                                .getId(), bean.getUser().getNickname(), Uri.parse(bean.getUser()
//                                .getPortraitUri()), bean.getDisplayName(), String.valueOf(bean
//                                .getStatus()), null, null, null, CharacterParser.getInstance()
//                                .getSpelling(bean.getUser().getNickname()), CharacterParser
//                                .getInstance().getSpelling(bean.getDisplayName())));
//                        // 通知好友列表刷新数据
//                        NToast.shortToast(mContext, R.string.agreed_friend);
//                        LoadDialog.dismiss(mContext);
//                        BroadcastManager.getInstance(mContext).sendBroadcast(SealAppContext
//                                .UPDATE_FRIEND);
