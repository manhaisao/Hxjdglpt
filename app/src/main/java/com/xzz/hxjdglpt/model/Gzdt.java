...
public class Gzdt implements Parcelable {
    ...
    public static final Creator<Gzdt> CREATOR = new Creator<Gzdt>() {
        public Gzdt createFromParcel(Parcel in) {
            return new Gzdt(in);
        }

        public Gzdt[] newArray(int size) {
            return new Gzdt[size];
        }
    };

    private Gzdt(Parcel in) {
        id = in.readLong();
        title = in.readString();
