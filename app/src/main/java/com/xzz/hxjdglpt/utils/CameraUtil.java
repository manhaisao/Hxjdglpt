...
public class CameraUtil {
    ...
    public static void openCamera(Activity activity) {
        ...
        // 判断存储卡是否可以用，可用进行存储
        if (hasSdcard()) {
            File tempFile = CompressImageUtil.createPicture(activity);
            ...
            if (currentapiVersion < 24) {
                ...
                Uri uri = activity.getApplication().getContentResolver().insert(MediaStore.Images
