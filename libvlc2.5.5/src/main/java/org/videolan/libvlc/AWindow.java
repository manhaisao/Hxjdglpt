...
public class AWindow implements IVLCVout {
    ...
    private class SurfaceHelper {
        ...
        private void attachSurface() {
            if (mSurfaceHolder != null) mSurfaceHolder.addCallback(mSurfaceHolderCallback);
            setSurface(mSurface);
        }
        ...
        private void releaseTextureView() {
            if (mTextureView != null) mTextureView.setSurfaceTextureListener(null);
        }

        public void release() {
            mSurface = null;
            setNativeSurface(mId, null);
            if (mSurfaceHolder != null) mSurfaceHolder.removeCallback(mSurfaceHolderCallback);
            releaseTextureView();
        }
        ...
        private final TextureView.SurfaceTextureListener mSurfaceTextureListener = AndroidUtil
                .isICSOrLater ? createSurfaceTextureListener() : null;
    }
    ...
    private SurfaceTextureThread mSurfaceTextureThread = AndroidUtil.isJellyBeanOrLater ? new
            SurfaceTextureThread() : null;
    ...
    private void ensureInitState() throws IllegalStateException {
        if (mSurfacesState.get() != SURFACE_STATE_INIT)
            throw new IllegalStateException("Can't set view when already attached. " + "Current " +
                    "state: " + mSurfacesState.get() + ", " + "mSurfaces[ID_VIDEO]: " +
                    mSurfaceHelpers[ID_VIDEO] + " / " + mSurfaces[ID_VIDEO] + ", " +
                    "mSurfaces[ID_SUBTITLES]: " + mSurfaceHelpers[ID_SUBTITLES] + " / " +
                    mSurfaces[ID_SUBTITLES]);
    }
    ...
    private void setView(int id, SurfaceView view) {
        ...
        if (surfaceHelper != null) surfaceHelper.release();
    ...
    private void setView(int id, TextureView view) {
        if (!AndroidUtil.isICSOrLater)
            throw new IllegalArgumentException("TextureView not implemented in this android " +
                    "version");
        ensureInitState();
        if (view == null) throw new NullPointerException("view is null");
        final SurfaceHelper surfaceHelper = mSurfaceHelpers[id];
    ...
    public void attachViews(OnNewVideoLayoutListener onNewVideoLayoutListener) {
        ...
        for (int id = 0; id < ID_MAX; ++id) {
            final SurfaceHelper surfaceHelper = mSurfaceHelpers[id];
            if (surfaceHelper != null) surfaceHelper.attach();
        }
    }
    ...
    public void detachViews() {
        if (mSurfacesState.get() == SURFACE_STATE_INIT) return;
        ...
        if (mSurfaceCallback != null) mSurfaceCallback.onSurfacesDestroyed(this);
        if (AndroidUtil.isJellyBeanOrLater) mSurfaceTextureThread.release();
    }
    ...
    private void onSurfaceCreated() {
        ...
        if (videoHelper.isReady() && (subtitlesHelper == null || subtitlesHelper.isReady())) {
            ...
            if (mSurfaceCallback != null) mSurfaceCallback.onSurfacesCreated(this);
        }
    }
    ...
    public void addCallback(IVLCVout.Callback callback) {
        if (!mIVLCVoutCallbacks.contains(callback)) mIVLCVoutCallbacks.add(callback);
    }
    ...
    private static native void nativeOnMouseEvent(long nativeHandle, int action, int button, int
            x, int y);
     ...
     */
    @SuppressWarnings("unused") /* used by JNI */ private Surface getVideoSurface() {
     ...
     * @return can be null if the surface was destroyed.
     */
    @SuppressWarnings("unused") /* used by JNI */ private Surface getSubtitlesSurface() {
    ...
    /**
     * Set a callback in order to receive {@link #nativeOnMouseEvent} and
     * {@link #nativeOnWindowSize} events.
     *
     * @param nativeHandle native Handle passed by {@link #nativeOnMouseEvent} and
     * {@link #nativeOnWindowSize}, cannot be NULL
     * @return true if callback was successfully registered
     */
    @SuppressWarnings("unused") /* used by JNI */ private int registerNative(long nativeHandle) {
        if (nativeHandle == 0) throw new IllegalArgumentException("nativeHandle is null");
        synchronized (mNativeLock) {
            if (mCallbackNativeHandle != 0) return AWINDOW_REGISTER_ERROR;
            mCallbackNativeHandle = nativeHandle;
            if (mMouseAction != -1)
                nativeOnMouseEvent(mCallbackNativeHandle, mMouseAction, mMouseButton, mMouseX,
                        mMouseY);
    ...
    @SuppressWarnings("unused") /* used by JNI */ private void unregisterNative() {
        synchronized (mNativeLock) {
    ...
    /**
     * This method is only used for ICS and before since ANativeWindow_setBuffersGeometry doesn't
     * work before.
     * It is synchronous.
     *
     ...
     */
    @SuppressWarnings("unused") /* used by JNI */ private boolean setBuffersGeometry(final
                                                                                     Surface surface, final int width, final int height, final int format) {
        if (AndroidUtil.isICSOrLater) return false;
        ...
        mHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                final SurfaceHolder surfaceHolder = surfaceHelper != null ? surfaceHelper
                        .getSurfaceHolder() : null;

                if (surfaceHolder != null) {
                    if (surfaceHolder.getSurface().isValid()) {
                        if (format != 0) surfaceHolder.setFormat(format);
     ...
     * This call will result
     * of{@link IVLCVout.OnNewVideoLayoutListener#onNewVideoLayout(IVLCVout, int, int, int, int, int, int)}
     ...
     */
    @SuppressWarnings("unused") /* used by JNI */ private void setVideoLayout(final int width,
                                                                              final int height,
                                                                              final int
                                                                                      visibleWidth, final int visibleHeight, final int sarNum, final int sarDen) {
        mHandler.post(new Runnable() {
            @Override
            public void run() {
                /* No need to synchronize here, mOnNewVideoLayoutListener is only set from
                MainThread */
                if (mOnNewVideoLayoutListener != null)
    ...
    @TargetApi(16)
    private static class SurfaceTextureThread implements Runnable, SurfaceTexture
        ...
        private boolean waitAndUpdateTexImage(float[] transformMatrix) {
            synchronized (this) {
                while (!mFrameAvailable) {
                    try {
                        wait(500);
                        if (!mFrameAvailable) return false;
                    } catch (InterruptedException ignored) {
                    }
                }
                mFrameAvailable = false;
     ...
     */
    @SuppressWarnings("unused") /* used by JNI */ private void SurfaceTexture_detachFromGLContext
    () {
     ...
     */
    @SuppressWarnings("unused") /* used by JNI */ private boolean
    SurfaceTexture_waitAndUpdateTexImage(float[] transformMatrix) {
     ...
     */
    @SuppressWarnings("unused") /* used by JNI */ private Surface SurfaceTexture_getSurface() {
